    ...
    <div className="max-w-4xl mx-auto px-6 py-20 animate-fade-in">
      ...
      <Card className="p-8 rounded-3xl space-y-6">
        <div>
          ...
          <p className="text-muted-foreground leading-relaxed">{settings.copyright_text}</p>
        </div>
        <div>
          <h2 className="text-xl font-semibold mb-3">DMCA / Khiếu Nại</h2>
          <p className="text-muted-foreground leading-relaxed">{settings.dmca_text}</p>
        </div>
        <div>
          <h2 className="text-xl font-semibold mb-3 flex items-center gap-2"><Mail className="w-5 h-5" />Liên Hệ</h2>
          <p className="text-muted-foreground">Nếu bạn có bất kỳ câu hỏi nào về bản quyền, vui lòng liên hệ với chúng tôi qua email: <a href="mailto:vietnam.tri@gmail.com" className="text-foreground underline underline-offset-4 hover:opacity-80 transition-opacity">vietnam.tri@gmail.com</a></p>
        </div>
      </Card>
    </div>
  );
