...
import { ArrowRight, Eye, ChevronRight, ChevronDown, MapPin, Calendar, Sparkles, Building2, User, Mountain, Landmark, Globe, Camera, Plane } from "lucide-react";
...
export default function HomePage() {
  ...
  return (
    <div>
      {/* Hero */}
      <section className="relative pt-4 pb-24 px-6">
          ...
          {heroAlbum && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-6">
              <Link href={`/albums/${heroAlbum.slug}`} className="lg:col-span-2 group">
                <Card className="relative h-[320px] sm:h-[400px] lg:h-[440px] overflow-hidden rounded-3xl border-0 cursor-pointer">
                  <BlurImage src={heroAlbum.cover_image_url} alt={heroAlbum.title} containerClassName="absolute inset-0" className="group-hover:scale-105 transition-transform duration-700" />
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-5 sm:p-8">
                    ...
                    <h1 className="text-2xl sm:text-3xl lg:text-5xl font-bold text-white mb-2 tracking-tight">{heroAlbum.title}</h1>
                    <p className="text-white/70 text-sm sm:text-base max-w-xl line-clamp-2 mb-4">{heroAlbum.description}</p>
              ...
              <div className="flex flex-col gap-4 lg:gap-6">
                {sideAlbums.map(a => (
                  <Link key={a.id} href={`/albums/${a.slug}`} className="group flex-1">
                    <Card className="relative h-full min-h-[155px] lg:min-h-[210px] overflow-hidden rounded-3xl border-0 cursor-pointer">
                      <BlurImage src={a.cover_image_url} alt={a.title} containerClassName="absolute inset-0" className="group-hover:scale-105 transition-transform duration-700" />
                      ...
                      <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-6">
                        ...
                        <h2 className="text-lg sm:text-xl font-bold text-white tracking-tight">{a.title}</h2>
                      </div>
                    </Card>
                  </Link>
                ))}
      ...
      <section className="py-16 px-6 border-t border-border/50">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <div className="flex items-center justify-center gap-3 mt-6">
            <Link href="/albums"><Button className="rounded-full gap-2">Khám Phá Albums <ArrowRight className="w-4 h-4" /></Button></Link>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3">
            {CATEGORIES.map(c => {
              ...
              return (
              <Link key={c.slug} href={`/categories/${c.slug}`} className="group flex flex-col items-center gap-2 p-4 rounded-2xl bg-card border border-border/50 hover:border-foreground/20 hover:shadow-sm transition-all duration-300">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {latest.map((album, i) => (
              <Link key={album.id} href={`/albums/${album.slug}`} className="group animate-fade-up" style={{ animationDelay: `${i * 100}ms` }}>
                <Card className="overflow-hidden rounded-3xl border-border/50 hover:border-foreground/10 hover:shadow-lg transition-all duration-300 h-full">
